...
reg [31:0] expected_repack [0:`REP_TILE_SOC_REPACK_WORDS-1];
...
initial begin
    ...
    $readmemh(`REP_TILE_SOC_EXPECTED_REPACK_HEX, expected_repack);
end
...
task print_summary;
    begin
        ...
        $display("  repack base word[0] = 0x%08h",
                 u_soc.u_dram.mem[`REP_TILE_SOC_REPACK_BASE >> 2]);
    end
endtask
...
function results_match;
    ...
    begin
        ...
        for (repack_i = 0; repack_i < `REP_TILE_SOC_REPACK_WORDS; repack_i = repack_i + 1) begin
            if (u_soc.u_dram.mem[(`REP_TILE_SOC_REPACK_BASE >> 2) + repack_i] !== expected_repack[repack_i])
                ok = 1'b0;
        end
        results_match = ok;
    end
...
task print_mismatch;
    ...
    begin
        ...
        for (repack_i = 0; repack_i < `REP_TILE_SOC_REPACK_WORDS; repack_i = repack_i + 1) begin
            if (!printed && u_soc.u_dram.mem[(`REP_TILE_SOC_REPACK_BASE >> 2) + repack_i] !== expected_repack[repack_i]) begin
                $display("  repack mismatch[%0d] got=0x%08h expected=0x%08h",
                         repack_i,
                         u_soc.u_dram.mem[(`REP_TILE_SOC_REPACK_BASE >> 2) + repack_i],
                         expected_repack[repack_i]);
                printed = 1'b1;
            end
        end
    end
