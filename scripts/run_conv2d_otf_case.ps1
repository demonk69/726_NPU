# =============================================================================
# run_conv2d_otf_case.ps1 - Generate and run one T6.2 Conv2D on-the-fly case
#
# Unlike T6.1, this stores raw IFM in DRAM. The NPU DMA generates the im2col
# activation row on the fly from Conv2D shape registers.
# =============================================================================
...
if (-not $Name) {
    $Name = ("conv2d_otf_{0}_{1}_b{2}_{3}x{4}_c{5}_k{6}x{7}_co{8}_p{9}x{10}" -f `
...
Write-Host "Generating $Name ($Dtype $Mode Conv2D on-the-fly im2col)" -ForegroundColor Cyan
$BiasArg = @()
if ($Bias) { $BiasArg = @("--bias") }
$QuantArg = @()
if ($Quant) { $QuantArg = @("--quant") }
if ($QuantRound) { $QuantArg += "--quant-round" }
& python "$ConvDir\gen_conv2d_im2col_data.py" --on-the-fly `
...
if ($LASTEXITCODE -ne 0) { throw "Failed to generate Conv2D on-the-fly data." }
