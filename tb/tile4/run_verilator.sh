...
run_icarus() {
    ...
    cd "$test_dir"
    timeout $TIMEOUT_SEC vvp "$vvp" 2>&1 | grep -E 'PASS|FAIL'
}
...
run_verilator() {
    ...
    cd "$test_dir"
    timeout $TIMEOUT_SEC "$vdir/Vtb_npu_tile_gemm_v2" 2>&1 | grep -E 'PASS|FAIL'
}
...
run_test() {
    ...
    local bias_flag=""
    local tag="${shape}_M${M}_K${K}_N${N}"
    ...
    python3 "$GEN_PY" --shape "$shape" --M "$M" --K "$K" --N "$N" \
        $bias_flag --out-dir "$out_dir" --name "$tag" 2>&1
...
run_all() {
    ...
    for entry in "${DEFAULT_SHAPES[@]}"; do
        ...
        run_test "$shape" "$M" "$K" "$N" "" "$mode"
        run_test "$shape" "$M" "$K" "$N" "--bias" "$mode"
    done
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        --all)       ALL=1 ;;
        --verilator) VERILATOR_ONLY=1; ALL=1 ;;
        --icarus)    ICARUS_ONLY=1; ALL=1 ;;
        --shape)     SHAPE="$2"; shift ;;
        --M)         M_VAL="$2"; shift ;;
        --K)         K_VAL="$2"; shift ;;
        --N)         N_VAL="$2"; shift ;;
        --bias)      BIAS_FLAG="--bias" ;;
        --help|-h)
            echo "Usage: $0 [--all] [--verilator|--icarus] [--shape S --M M --K K --N N] [--bias]"
            exit 0 ;;
        *) die "Unknown arg: $1" ;;
    esac
    shift
...
if [[ "$ALL" -eq 1 ]]; then
    ...
    run_test "$SHAPE" "$M_VAL" "$K_VAL" "$N_VAL" "$BIAS_FLAG" "icarus"
    echo ""
    run_test "$SHAPE" "$M_VAL" "$K_VAL" "$N_VAL" "$BIAS_FLAG" "verilator"
else
