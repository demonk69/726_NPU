...
localparam DESC_FLAG_FIRST_K        = (32'h1 << 17);
localparam DESC_FLAG_LAST_K         = (32'h1 << 18);
localparam DESC_FLAG_LAST_LAYER     = (32'h1 << 19);
localparam DESC_FLAG_IRQ_EN         = (32'h1 << 20);
localparam DESC_FLAG_IFM_PREV_OFM   = (32'h1 << 23);
...
localparam DESC_CTRL_CHAIN_LAST = DESC_CTRL_COMMON |
                                  DESC_FLAG_IFM_PREV_OFM |
                                  DESC_FLAG_IRQ_EN |
                                  DESC_FLAG_LAST_LAYER;
...
integer ar_count;
integer aw_count;
reg [31:0] ar_addr_seen [0:31];
reg [7:0]  ar_len_seen  [0:31];
...
function integer a0_val;
    input integer r;
    input integer k;
    begin
        a0_val = (((r + 1) * 3 + (k + 2) * 2) % 11) - 5;
    end
endfunction

function integer w0_val;
    input integer k;
    input integer c;
    begin
        w0_val = (k == c) ? 1 : 0;
...
task init_matrices;
    ...
    begin
        for (k = 0; k < K_DIM; k = k + 1) begin
            dram[(A0_ADDR >> 2) + k] = pack4_int8(
                a0_val(0, k), a0_val(1, k), a0_val(2, k), a0_val(3, k));
            dram[(W0_ADDR >> 2) + k] = pack4_int8(
                w0_val(k, 0), w0_val(k, 1), w0_val(k, 2), w0_val(k, 3));
            dram[(W1_ADDR >> 2) + k] = pack4_int8(
                w1_val(k, 0), w1_val(k, 1), w1_val(k, 2), w1_val(k, 3));
        end
        ...
        for (r = 0; r < M_DIM; r = r + 1) begin
            for (c = 0; c < N_DIM; c = c + 1) begin
                ...
                for (k = 0; k < K_DIM; k = k + 1)
                    acc = acc + a0_val(r, k) * w0_val(k, c);
                expected0[r*N_DIM + c] = acc;
                dram[(R0_ADDR >> 2) + r*N_DIM + c] = 32'hdead_1000 + r*N_DIM + c;

                acc = 0;
                for (k = 0; k < K_DIM; k = k + 1)
                    acc = acc + a0_val(r, k) * w1_val(k, c);
                expected1[r*N_DIM + c] = acc;
                dram[(R1_ADDR >> 2) + r*N_DIM + c] = 32'hdead_2000 + r*N_DIM + c;
            end
        end
    end
endtask
...
task wait_done;
    ...
    begin
        ...
        if (!status[1]) begin
            $display("[FAIL] descriptor chain timeout state=%0d dma=%0d",
...
always @(posedge clk) begin
    if (!rst_n) begin
        ...
        m_arready <= 1'b1;
        if (m_arvalid && m_arready && ar_count < 32) begin
...
initial begin
    ...
    write_desc(DESC1_ADDR, DESC_CTRL_CHAIN_LAST, 32'd0, W1_ADDR, R1_ADDR, 32'd0);
    init_matrices();
    ...
    if (ar_count !== 21) begin
        $display("[FAIL] expected 21 descriptor/W/A/OFM read bursts, got %0d", ar_count);
        fail_cnt = fail_cnt + 1;
    end else begin
        pass_cnt = pass_cnt + 1;
    end
    ...
    for (k = 0; k < K_DIM; k = k + 1) begin
        for (r = 0; r < M_DIM; r = r + 1) begin
            expect_read(i, R0_ADDR + ((r * K_DIM + k) * 4), 8'd0);
            i = i + 1;
        end
    end
    ...
    if (fail_cnt == 0) begin
        $display("[PASS] tb_npu_desc_ofm_chain: ALL %0d CHECKS PASSED", pass_cnt);
    end else begin
        $display("[FAIL] tb_npu_desc_ofm_chain: %0d passed, %0d failed",
                 pass_cnt, fail_cnt);
        $fatal;
    end
...
initial begin
    #(CLK_T * 300000);
    $display("[FAIL] tb_npu_desc_ofm_chain global timeout");
    $fatal;
end
