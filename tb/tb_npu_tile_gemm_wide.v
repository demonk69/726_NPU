...
`ifdef CFG_SHAPE_VAL
localparam CFG_SHAPE_V   = `CFG_SHAPE_VAL;
`else
localparam CFG_SHAPE_V   = 32'h0;
`endif
`ifdef GRID_COLS_VAL
localparam GRID_COLS_V   = `GRID_COLS_VAL;
`else
localparam GRID_COLS_V   = 4;
`endif
`ifdef AW_EXPECT_VAL
localparam AW_EXPECT_V   = `AW_EXPECT_VAL;
`else
localparam AW_EXPECT_V   = 4;
`endif
        ...
        if (`IS_FP16) begin
            if (fp32_close(got, exp)) begin
                ...
                $display("[FAIL] %s C[%0d][%0d] got=0x%08h exp=0x%08h",
                         `TEST_NAME, idx / GRID_COLS_V, idx % GRID_COLS_V, got, exp);
                fail_cnt = fail_cnt + 1;
            end
        end else begin
            if (got === exp) begin
                ...
                $display("[FAIL] %s C[%0d][%0d] got=%0d (0x%08h) exp=%0d (0x%08h)",
                         `TEST_NAME, idx / GRID_COLS_V, idx % GRID_COLS_V,
...
initial begin
    ...
    axi_write(REG_CFG_SHAPE, CFG_SHAPE_V);
`ifdef BIAS_ADDR_VAL
    axi_write(32'h98, `BIAS_ADDR_VAL);
`endif
`ifdef QUANT_CFG_VAL
    axi_write(32'h9C, `QUANT_CFG_VAL);
`endif
`ifdef CTRL_VAL
...
`endif
    ...
    if (aw_count !== AW_EXPECT_V) begin
        $display("[FAIL] %s expected %0d row write bursts, got %0d", `TEST_NAME, AW_EXPECT_V, aw_count);
        $finish;
    end
