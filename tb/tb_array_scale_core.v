// =============================================================================
// tb_array_scale_core.v - Legacy core for 1x1 PE K-depth verification
//
// This file is included by per-K wrappers (historical naming retained).
// Do NOT compile this file directly.
//
// Required defines from wrapper:
//   N         - K dimension (dot-product length)
//   DRAM_PATH - full path to dram_init.hex
//   EXP_PATH  - full path to expected.hex
//   NUM_TESTS - number of test cases
//   DRAM_SIZE - DRAM array size
//   T<n>_*    - per-test parameters (W_ADDR, A_ADDR, R_ADDR, CTRL, K_DIM,
//               IS_FP16, IS_OS)
//
// Architecture note:
//   Uses ROWS=1, COLS=1 to isolate computation correctness. K is varied
//   (K=N) to test dot-product depth.
//   Current RTL semantics:
//     WS : internal ws_acc accumulates the full K-element dot product and
//          flushes one FP32 / INT32 result word.
//     OS : internal os_acc accumulates the full K-element dot product and
//          flushes one FP32 / INT32 result word.
//   Current PPBuf packing: INT8 uses SUBW=4 (4 elements per 32-bit word);
//   FP16 remains standard 2-per-32-bit-word packing.
// =============================================================================
...
localparam ROWS    = 1;     // Fixed 1 row
localparam COLS    = 1;     // Fixed 1 column (avoid multi-col FIFO issues)
...
// ---------------------------------------------------------------------------
// FP32 ULP comparison (tolerant NaN/Inf handling)
// ---------------------------------------------------------------------------
...
task do_test;
    ...
    begin
        ...
        // Debug: print DRAM words at result address
        $display("  DBG T%0d: dram[%0d..%0d] = %0d %0d %0d %0d %0d %0d",
                 idx, r_addr>>2, (r_addr>>2)+5,
                 $signed(dram[r_addr>>2]),   $signed(dram[(r_addr>>2)+1]),
                 $signed(dram[(r_addr>>2)+2]),$signed(dram[(r_addr>>2)+3]),
                 $signed(dram[(r_addr>>2)+4]),$signed(dram[(r_addr>>2)+5]));
...
initial begin
    ...
    $display("");
    $display("################################################################");
    $display("  PE Array Scale Verification: K=%0d (ROWS=1, COLS=1)", `N);
    $display("  Running %0d tests...", `NUM_TESTS);
    $display("################################################################");

    `ifdef TEST_0
    $display(""); $display("--- Test 0: %0s%0s K=%0d ---",
             `T0_IS_FP16 ? "FP16" : "INT8", `T0_IS_OS ? "_OS" : "_WS", `T0_K_DIM);
    ...
    `ifdef TEST_1
    $display(""); $display("--- Test 1: %0s%0s K=%0d ---",
             `T1_IS_FP16 ? "FP16" : "INT8", `T1_IS_OS ? "_OS" : "_WS", `T1_K_DIM);
    ...
    `ifdef TEST_2
    $display(""); $display("--- Test 2: %0s%0s K=%0d ---",
             `T2_IS_FP16 ? "FP16" : "INT8", `T2_IS_OS ? "_OS" : "_WS", `T2_K_DIM);
    ...
    `ifdef TEST_3
    $display(""); $display("--- Test 3: %0s%0s K=%0d ---",
             `T3_IS_FP16 ? "FP16" : "INT8", `T3_IS_OS ? "_OS" : "_WS", `T3_K_DIM);
    ...
    `ifdef TEST_4
    $display(""); $display("--- Test 4: %0s%0s K=%0d ---",
             `T4_IS_FP16 ? "FP16" : "INT8", `T4_IS_OS ? "_OS" : "_WS", `T4_K_DIM);
    ...
    `ifdef TEST_5
    $display(""); $display("--- Test 5: %0s%0s K=%0d ---",
             `T5_IS_FP16 ? "FP16" : "INT8", `T5_IS_OS ? "_OS" : "_WS", `T5_K_DIM);
    ...
    `ifdef TEST_6
    $display(""); $display("--- Test 6: %0s%0s K=%0d ---",
             `T6_IS_FP16 ? "FP16" : "INT8", `T6_IS_OS ? "_OS" : "_WS", `T6_K_DIM);
    ...
    `ifdef TEST_7
    $display(""); $display("--- Test 7: %0s%0s K=%0d ---",
             `T7_IS_FP16 ? "FP16" : "INT8", `T7_IS_OS ? "_OS" : "_WS", `T7_K_DIM);
    ...
    `ifdef TEST_8
    $display(""); $display("--- Test 8: %0s%0s K=%0d ---",
             `T8_IS_FP16 ? "FP16" : "INT8", `T8_IS_OS ? "_OS" : "_WS", `T8_K_DIM);
    ...
    `ifdef TEST_9
    $display(""); $display("--- Test 9: %0s%0s K=%0d ---",
             `T9_IS_FP16 ? "FP16" : "INT8", `T9_IS_OS ? "_OS" : "_WS", `T9_K_DIM);
    ...
    `ifdef TEST_10
    $display(""); $display("--- Test 10: %0s%0s K=%0d ---",
             `T10_IS_FP16 ? "FP16" : "INT8", `T10_IS_OS ? "_OS" : "_WS", `T10_K_DIM);
    ...
    `ifdef TEST_11
    $display(""); $display("--- Test 11: %0s%0s K=%0d ---",
             `T11_IS_FP16 ? "FP16" : "INT8", `T11_IS_OS ? "_OS" : "_WS", `T11_K_DIM);
    ...
    $display("################################################################");
    if (fail_cnt == 0)
        $display("  ALL %0d TESTS PASSED (K=%0d)", pass_cnt, `N);
    else
...
// Debug trace: print key signals each cycle when NPU is active
// Uses only signals that exist regardless of COLS value (no ser_active).
reg [31:0] dbg_cycle;
initial dbg_cycle = 0;
always @(posedge clk) begin
    dbg_cycle <= dbg_cycle + 1;
    if (rst_n && dbg_cycle < 500) begin
        if (u_npu.u_ctrl.state != 0 || u_npu.u_dma.dma_state != 0) begin
            $display("  [T%0d] ctrl=%0d dma=%0d pe_en=%0b flush=%0b stat=%0b pe_valid=%0b acc=%0d fifo_wr=%0b fifo_din=%0d w_vld=%0b wdata=%0d",
                     dbg_cycle,
                     u_npu.u_ctrl.state,
                     ...
                     $signed(u_npu.u_pe_array.gen_direct_pe_grid.gen_row[0].gen_col[0].u_pe.acc_out),
                     u_npu.r_fifo_wr_en,
                     $signed(u_npu.r_fifo_din),
                     u_npu.m_axi_wvalid,
                     $signed(u_npu.m_axi_wdata));
        end
    end
end
