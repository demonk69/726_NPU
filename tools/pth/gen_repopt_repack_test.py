...
def build_case(plan, plan_dir, state_dict, args):
    ...
    layout = {"a_base": a_base, "w_base": w_base, "bias_base": bias_base,
              ...
              "ifm_base": q_base + align(total_m * total_n * 4, 0x100),
              "ifm_words": align(total_n * total_m, 4) // 4,
              "oh": oh, "ow": ow}
...
def assemble_firmware_global(layout, args):
    ...
    # ── Repack: row-major q[m][n] → NCHW int8 IFM ──
    # For each channel c (0..total_n0-1):
    #   For each pixel m (0..total_m0-1):
    #     byte = q0_base + m*total_n0*4 + c*4
    #     write to ifm_base + c*total_m0 + m
    total_m0 = layout["total_m"]; total_n0 = layout["total_n"]
    ifm_base = layout["q_base"] + align(total_m0 * total_n0 * 4, 0x100)
    verify_addr = ifm_base + align(total_n0 * total_m0, 0x100)  # after IFM area

    emit(*li_insns("a4", total_n0))       # channel count
    emit(*li_insns("a5", 0))              # c = 0
    label("rep_c")
    emit(SLLI("a2", "a5", 2))             # a2 = c * 4
    emit(*li_insns("t2", layout["q_base"])); emit(ADD("t2", "t2", "a2"))  # t2 = q0 + c*4
    emit(*li_insns("a2", total_m0)); emit(MUL("a2", "a5", "a2"))  # a2 = c * total_m0
    emit(*li_insns("a1", ifm_base)); emit(ADD("a1", "a1", "a2"))  # a1 = ifm + c*total_m0
    emit(*li_insns("a6", total_m0))       # m counter
    emit(*li_insns("t6", total_n0 * 4))   # stride between q rows
    label("rep_m")
    emit(LW("t1", "t2", 0))               # q word
    emit(SB("t1", "a1", 0))               # store low byte to IFM
    emit(ADD("t2", "t2", "t6"))           # q_ptr += total_n0*4 (next pixel, same channel)
    emit(ADDI("a1", "a1", 1))             # ifm_ptr += 1 (next pixel)
    ...
    # Write first 16 IFM bytes to verify area for testbench check
    emit(*li_insns("t2", ifm_base))
    emit(*li_insns("t0", verify_addr))
    emit(LW("t1", "t2", 0)); emit(SW("t1", "t0", 0))
    emit(LW("t1", "t2", 4)); emit(SW("t1", "t0", 4))
    emit(LW("t1", "t2", 8)); emit(SW("t1", "t0", 8))
    emit(LW("t1", "t2", 12)); emit(SW("t1", "t0", 12))
...
def write_params_global(out_dir, fw_words, args, layer, label, in_scale, in_zp, tiles, layout, exp_q):
    ...
    lines = [
        ...
        f"`define REP_TILE_SOC_IFM1_BASE 32'h{layout['ifm_base']:08x}",
        f"`define REP_TILE_SOC_IFM1_WORDS {layout['ifm_words']}",
    ]
