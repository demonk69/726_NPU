#!/usr/bin/env bash
# Run multi-core runtime closed-loop RepOpt VGG classification.
#
# Usage:
#   ./run_vgg_mc_closed_loop.sh
#   ./run_vgg_mc_closed_loop.sh --num-cores 2
#   ./run_vgg_mc_closed_loop.sh --num-cores 1 --image cat.jpg
#   ./run_vgg_mc_closed_loop.sh --num-cores 2 --shape 16x16 --flow os
#   ./run_vgg_mc_closed_loop.sh --num-cores 2 --shape 4x4 --flow ws --image cat.jpg
#   ./run_vgg_mc_closed_loop.sh --num-cores 2 --lanes 2
set -euo pipefail

ROOT="$(cd "$(dirname "$0")" && pwd)"
OUT_DIR="$ROOT/sim/vgg_mc_closed_loop"
TIMEOUT_CYCLES="${VGG_CLOSED_TIMEOUT_CYCLES:-500000000}"
RUN_TIMEOUT_SECONDS="${VGG_MC_SHELL_TIMEOUT_SECONDS:-14400}"
...
echo "=== Generate Multi-Core Closed Loop (cores=$NUM_CORES) ==="
python3 -B "$ROOT/tools/pth/gen_vgg_closed_loop.py" --out-dir "$OUT_DIR" "${GEN_ARGS[@]}"

echo "=== Compile ==="
VL_ARGS=("--binary" "--timing" "-I$OUT_DIR" "-Mdir" "$OUT_DIR/obj_dir" "--top-module" "tb_soc_mc_vgg_closed_loop"
...
verilator "${VL_ARGS[@]}" \
  ...
  "$ROOT/rtl/soc/"*.v \
  "$ROOT/rtl/top/"*.v \
  "$ROOT/tb/tb_soc_mc_vgg_closed_loop.v"

echo "=== Run ==="
LOG_FILE="$OUT_DIR/run.log"
set +e
timeout "$RUN_TIMEOUT_SECONDS" stdbuf -oL -eL "$OUT_DIR/obj_dir/Vtb_soc_mc_vgg_closed_loop" 2>&1 | tee "$LOG_FILE"
RUN_RC=${PIPESTATUS[0]}
set -e
