...
| OUT/OFM | 最后一个 k_tile 后的输出。当前 RTL 主路径写回 32-bit accumulator word，后续再接 bias/activation/quant。 |
...
直接寄存器模式从 CPU 写 `M_DIM/N_DIM/K_DIM/W_ADDR/A_ADDR/R_ADDR/ARR_CFG/CFG_SHAPE/CTRL.start` 开始，`npu_ctrl` 直接使用这些 shadow config 调度 tile。
...
| `npu_top` | `rtl/top/npu_top.v` | 标量兼容路径、4x4 tile-mode GEMM、descriptor fetch 和 T5.4 OFM->IFM 接线已通过 | 连接 DMA、buffer、阵列、结果收集 |
| `npu_axi_lite` | `rtl/axi/npu_axi_lite.v` | 寄存器文件和 descriptor 提交寄存器可用 | CPU 配置、状态、中断、descriptor base/count |
| `npu_ctrl` | `rtl/ctrl/npu_ctrl.v` | 标量 loop、4x4 tile planner、descriptor fetch/decode/next-layer、上一层 OFM 地址跟踪 | descriptor/tile/layer 调度 |
| `npu_dma` | `rtl/axi/npu_dma.v` | 读/写侧 INCR burst、4KB 边界切分、64-byte descriptor fetch、OFM row-major gather/repack | 多通路 INCR burst DMA 和带宽统计 |
...
T5.4 后，`IFM_FROM_PREV_OFM=0` 时每层仍是独立 GEMM 任务，`ifm_addr/weight_addr/ofm_addr` 都由 descriptor 明确给出；`IFM_FROM_PREV_OFM=1` 时，`npu_ctrl` 使用上一 descriptor 的 `ofm_addr` 覆盖本层 A 源地址，并给 DMA 传入 `m_base/k_base/k_len/active_rows/stride`，由 DMA 从 row-major 32-bit OFM surface gather 出 A tile lane。该路径当前验证 INT8；FP16 仍需要明确的格式转换或后处理。
...
T5.3 新增的 descriptor fetch 复用读侧 AXI 通道，但输出目的不是 PPBuf，而是 `desc_words[511:0]`。因此 DMA 读侧需要区分“descriptor read”和“tile data read”两类目的地，避免把 descriptor 当成 A/W 数据写进 buffer。T5.4 又增加 A_OFM 读目标：它从上一层 row-major 32-bit OFM 发起每 lane 单 beat read，低 8 bit 被打包成 INT8 A lane 后写入 A PPBuf。

目标：

1. 支持 descriptor、W、A、A_FROM_OFM、PSUM、OUT 多种传输类型。
