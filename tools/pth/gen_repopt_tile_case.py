...
def build_tile_matrices(x_q, qweight, layer, m_base, n_base):
    k_dim = int(layer["registers"]["K_DIM"])
    cout = int(layer["weight_shape"][0])
    total_m = int(layer["registers"]["M_DIM"])
    if m_base < 0 or m_base + 4 > total_m:
        raise ValueError(f"m-base {m_base} does not fit one 4-row tile in M={total_m}")
    if n_base < 0 or n_base + 4 > cout:
        raise ValueError(f"n-base {n_base} does not fit one 4-col tile in Cout={cout}")
    ...
    for k_index in range(k_dim):
        a_tile.append([conv_tile_a_value(x_q, layer, m_base + r, k_index) for r in range(4)])
        chan = k_index // (kh * kw)
        rem_k = k_index % (kh * kw)
        ker_h = rem_k // kw
        ker_w = rem_k % kw
        w_tile.append([int(weight_int[n_base + c, chan, ker_h, ker_w].item()) for c in range(4)])
    return a_tile, w_tile


def expected_tile(a_tile, w_tile):
    out = []
    for r in range(4):
        for c in range(4):
            acc = 0
