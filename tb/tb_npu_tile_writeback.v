...
task expect_word;
    ...
    input [31:0] exp;
    begin
        if (dram[addr >> 2] !== exp) begin
            $display("[FAIL] C word at 0x%08h got %0d (0x%08h), expected %0d",
                     addr, $signed(dram[addr >> 2]), dram[addr >> 2], $signed(exp));
            $finish;
        end
    end
endtask
...
initial begin
    ...
    if (aw_count !== 4) begin
        $display("[FAIL] expected 4 row write bursts, got %0d", aw_count);
        $finish;
    end
    for (i = 0; i < 4; i = i + 1) begin
        if (aw_addr_seen[i] !== (32'h180 + i*16) || aw_len_seen[i] !== 8'd3) begin
            $display("[FAIL] row burst %0d addr=%08h len=%0d", i, aw_addr_seen[i], aw_len_seen[i]);
            $finish;
        end
    end

    expect_word(32'h180, 32'd5);
    expect_word(32'h184, 32'd6);
    expect_word(32'h188, 32'd7);
    expect_word(32'h18C, 32'd8);
    expect_word(32'h190, 32'd10);
    expect_word(32'h194, 32'd12);
    expect_word(32'h198, 32'd14);
    expect_word(32'h19C, 32'd16);
    expect_word(32'h1A0, 32'd15);
    expect_word(32'h1A4, 32'd18);
    expect_word(32'h1A8, 32'd21);
    expect_word(32'h1AC, 32'd24);
    expect_word(32'h1B0, 32'd20);
    expect_word(32'h1B4, 32'd24);
    expect_word(32'h1B8, 32'd28);
    expect_word(32'h1BC, 32'd32);

    $display("[PASS] tb_npu_tile_writeback: 4x4 tile wrote 16 row-wise words");
    $finish;
end
