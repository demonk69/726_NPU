...
> 最后更新：2026-04-14
> 状态：已按可重配置 PE 阵列（16×16 + cfg_shape 四形态）与双权重寄存器升级同步
...
3. [快速开始](#3-快速开始)
4. [寄存器编程指南](#4-寄存器编程指南)
5. [PE 阵列形状配置（cfg_shape）](#5-pe-阵列形状配置cfg_shape)
6. [数据布局与地址约定](#6-数据布局与地址约定)
7. [端到端推理调度详解：三层卷积+激活](#7-端到端推理调度详解三层卷积激活)
8. [SoC 集成使用说明](#8-soc-集成使用说明)
9. [常用仿真入口](#9-常用仿真入口)
10. [常见问题](#10-常见问题)
11. [配套文档索引](#11-配套文档索引)
...
- **16×16 可重配置 PE 阵列**（通过 `cfg_shape` 运行时切换 4×4 / 8×8 / 16×16 / 8×32）
- INT8 / FP16 数据通路
- WS / OS 两种计算模式
- AXI4-Lite 配置接口
- AXI4 DMA 数据搬运
...
| OS 模式 | ✅ 已验证 |
| SoC 固件驱动 | ✅ 已验证 |
| 全量回归 | ✅ 51/51 PASS（2026-04-14 重构后） |

### 当前最重要的理解方式

这个工程现在不是"整块 16×16 输出并行一次算完"的经典说明书模型，而是：
...
## 5. PE 阵列形状配置（cfg_shape）

> **2026-04-14 新增**。NPU 现在使用 16×16 物理可重配置 PE 阵列，通过 `cfg_shape` 寄存器在运行时切换工作形态。
...
| **位域** | `[1:0]` |
| **类型** | R/W |
| **锁存时机** | `start` 脉冲上升沿（锁存为 `lk_shape`） |

### 5.2 四种工作形态

| 值 | 形态 | 活跃 PE | 适用场景 |
|---:|------|--------:|---------|
| `2'b00` | **4×4** | 左上角 16 个 | 小模型、极低功耗、快速验证 |
| `2'b01` | **8×8** | 左上角 64 个 | 中等规模推理 |
| `2'b10` | **16×16** | 全部 256 个 | 大矩阵并行计算 |
| `2'b11` | **8×32 折叠** | 全部 256 个（对折为逻辑 8×32） | 宽输出向量 / 大 N 维度矩阵乘法 |

### 5.3 使用方法

#### 步骤

1. **先配置 shape，再启动运算**（顺序重要！）

```c
// 示例：配置 8×8 模式后启动 INT8 WS 计算
NPU_REG(0x3C) = 0x01;           // cfg_shape = 8x8
NPU_REG(0x18) = K_DIM;          // K 维度
NPU_REG(0x10) = M_DIM;          // M 维度
NPU_REG(0x14) = N_DIM;          // N 维度
NPU_REG(0x20) = W_ADDR_BASE;    // 权重基地址
NPU_REG(0x24) = A_ADDR_BASE;    // 激活基地址
NPU_REG(0x28) = R_ADDR_BASE;    // 结果基地址
NPU_REG(0x00) = 0x01;           // start=1, mode=INT8, stat=WS
```

2. **等待完成**

```c
while (NPU_REG(0x04) & 0x02) {}   // 等待 done
NPU_REG(0x00) = 0x00;            // 清除 start
```

### 5.4 8×32 折叠模式详解

当 `cfg_shape = 2'b11` 时，物理 16×16 阵列被**对折**为逻辑 8×32 阵列：

```
物理布局 (16行 × 16列):
┌──────────────────────┐
│ Top Half (Row 0~7)   │ → 输出 L[0..15]
│   Col 0~14 + fold    │
│                      │
│ Bottom Half (Row~15) │ → 输出 R[16..31]
│   fold_in + Col~15   │
└──────────────────────┘

折叠连接:
  Row7,Col15 的激活输出 ──► Row8,Col0 的激活输入
  下半部的部分和输入 = 0（无垂直链路）
```

**使用注意**：
- 8×32 模式的逻辑列数是 32（两半各贡献 16 列）
- 输出结果宽度：`acc_out` 为 `32 × ACC_W = 1024 bit`
- 适合 N 维度较大但 M 维度较小的场景（如全连接层）

### 5.5 功耗说明

非活跃区域的 PE 接收 `pe_clk_en = 0`：
- 内部寄存器冻结（不翻转）
- 不产生动态功耗
- 时钟树到该区域仍可由后端 ICG 完全关断
