...
| 模块 | 文件 | 当前状态 | 目标职责 |
|---|---|---|---|
| `pe_top` | `rtl/pe/pe_top.v` | 单 PE 测试通过，T7.3/T7.4 INT8 2/4-lane SIMD 已验证 | INT8/FP16 MAC，WS/OS 累加 |
...
- `mode=0`：INT8 signed MAC。`DATA_W=16` packed 输入执行 2-lane dot，`DATA_W=32` packed 输入执行 4-lane dot，再做 32-bit accumulate；旧 sign-extended scalar 输入保持单 lane 兼容。
...
T7.4 后，`pe_top` 的 INT8 packed lane 约定为低字节优先：`DATA_W=16` 使用 `{lane1,lane0}`，`DATA_W=32` 使用 `{lane3,lane2,lane1,lane0}`。为兼容旧 direct scalar/PPBuf feeder，当 W/A 两侧的高位都只是 lane0 的符号扩展时，PE 只计算 lane0；因此 PE 级 2/4-lane SIMD 已可验证，但端到端 2x/4x 吞吐还需要上游生成非 sign-extended 的 packed K lane 数据。
...
2. 将 32-bit packed K lane 供数接到阵列/top，并配套 valid 对齐、边界 mask 和写回。
...
- T7.2 已验证阵列级 8x32 折叠路由：top half 映射逻辑列 0..15，bottom half 映射逻辑列 16..31，32-lane 输出顺序正确。

当前限制：

- 顶层 tile feeder 已具备 4/8/16 lane 供数并验证 8x8/16x16 active lane；完整 8x8/16x16 结果收集/写回仍待补齐。
- OS 模式的 4x4 路径已通过 row-skew feeder 和 GEMM golden；8x32 目前只完成阵列级折叠路由验证，顶层 32-output 写回和端到端 golden 仍待补齐。
...
downstream: reconfig_pe_array exposes result_vec[32]/valid_vec[32]; current npu_top completed writeback path consumes the 4x4 result subset
```

当前 4x4 tile 使用左上角 PE(0..3,0..3) 并完成写回 golden；8x8/16x16 已验证 active lane 供数；8x32 已验证阵列级折叠输出顺序。完整高吞吐矩阵乘仍需要把更大形态的结果收集、边界 mask 和写回映射接到顶层。
...
- `op_counter` 已接入 controller/compute 信号，T7.5 的 MAC/OPS、`TOPS_X1E6`、compute/e2e utilization 和 peak ops/cycle 通过 `npu_axi_lite` 暴露。
